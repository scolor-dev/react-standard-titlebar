 ...
 * ThemeContext
 *
 * [EN] React Context that stores the current theme state.
 * Must be wrapped with ThemeProvider.
 *
 * [JP] 現在のテーマ状態を保持するReact Context。
 * 必ずThemeProviderでラップして使用します。
 */
...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({
  ...
  useEffect(() => {
    ...
    if (effectiveTheme === 'custom') {
      ...
      const applyButtonVars = (prefix: string, btn?: Record<string, string | undefined>) => {
        if (!btn) return;
        Object.entries(btn).forEach(([key, value]) => {
          if (value) {
            const cssVar = `--btn-${prefix}${key.replace(/([A-Z])/g, '-$1').toLowerCase()}`;
            document.documentElement.style.setProperty(cssVar, value);
          }
        });
      };

      applyButtonVars('', buttons?.base);
      applyButtonVars('min-', buttons?.min);
      applyButtonVars('max-', buttons?.max);
      applyButtonVars('exit-', buttons?.exit);
    } else {
  ...
  }, [theme, headerBg, logoColor, buttons]);
